...
import {
  FiMonitor, FiTrendingUp, FiShoppingCart, FiBox, FiCheck, FiBarChart2, FiLayers
} from "react-icons/fi";
...
const Banner = () => {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center w-full">
          ...
          <div>
            ...
            <h1 className="text-3xl md:text-6xl leading-[1.2] font-bold tracking-tight">
              Enterprise-ready tools <br/>for modern retail & eCommerce
            </h1>

            {/* Description */}
            <p className="mt-8 text-lg md:text-xl text-gray-400 leading-relaxed font-light max-w-2xl">
              Manage your retail stores, online orders, inventory, POS,
              customer experience, and AI-powered commerce operations from one
              powerful platform.
            </p>
  ...
  <div className="relative w-full max-w-[900px] rounded-[50px] border border-slate-200 bg-white p-5 shadow-[0_40px_100px_-20px_rgba(0,0,0,0.08)]">
    
    {/* 1. TOP FLOATING CARD: Sales (Shopify Style) */}
    <div className="absolute -top-20 -right-10 z-30 rounded-[30px] border border-slate-200 bg-white p-7 shadow-2xl transition-all hover:-translate-y-2 duration-500">
      <div className="flex items-center gap-3 mb-3">
        <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-blue-50 text-blue-600">
          <FiTrendingUp size={20} />
        </div>
        <p className="text-[11px] font-bold uppercase tracking-widest text-slate-400">Net Sales</p>
      </div>
    ...
    <div className="overflow-hidden rounded-[40px] border border-slate-100 bg-[#f8f9fa] p-8 min-h-[420px]">
      
      {/* Dashboard Header Simulation */}
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-6">
          {['Analytics', 'Stock', 'Orders'].map((item, i) => (
            <span key={i} className={`text-xs font-bold uppercase tracking-widest cursor-pointer ${i === 0 ? 'text-blue-600 border-b-2 border-blue-600 pb-1' : 'text-slate-400 hover:text-slate-600'}`}>
              {item}
            </span>
          ))}
        </div>
      ...
      <div className="grid grid-cols-12 gap-8">
        ...
        <div className="col-span-7">
          <div className="bg-white rounded-3xl p-6 border border-slate-200 h-full">
            <div className="flex justify-between items-center mb-8">
               <h5 className="text-sm font-semibold text-slate-800">Sales Turnover</h5>
               <FiBarChart2 className="text-slate-300" />
            </div>
            <div className="flex items-end justify-between h-32 gap-2">
              {[60, 40, 90, 70, 50, 85, 65].map((h, i) => (
                <div key={i} className="w-full bg-blue-50 rounded-t-lg relative group transition-all" style={{ height: `${h}%` }}>
        ...
        <div className="col-span-5 space-y-4">
          <div className="p-5 rounded-3xl border border-slate-200 bg-white  flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] font-semibold text-slate-400 uppercase">Low Stock Alert</p>
              <p className="text-sm font-semibold text-slate-900">12 Products</p>
            </div>
          </div>
          ...
          <div className="p-5 rounded-3xl bg-white border border-slate-200 flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] font-semibold text-slate-400 uppercase">Automation</p>
              <p className="text-sm font-semibold text-slate-900">Stock Syncing</p>
            </div>
          </div>

          <div className="p-4 px-6 rounded-full bg-slate-900 flex items-center justify-between group cursor-pointer">
             <span className="text-sm font-normal text-white">View Reports</span>
    ...
    <div className="absolute -bottom-16 -left-10 z-30 rounded-[30px] border border-slate-200 bg-white p-6 shadow-2xl transition-all hover:scale-105 duration-500">
      <div className="flex items-center gap-5">
        <div className="flex h-16 w-16 items-center justify-center rounded-[24px] bg-blue-600 text-white shadow-lg shadow-blue-200">
          <FiShoppingCart size={28} />
        </div>
        <div>
          <p className="text-[11px] font-bold uppercase tracking-widest text-slate-400">Orders Processed</p>
          <h4 className="text-3xl font-bold tracking-tight text-slate-900">12,840+</h4>
        </div>
      </div>
    </div>
